...
- Bump the toolchain baseline in the 2026-08-04 monthly maintenance cycle: Gradle `9.6.0` → `9.6.1`, Node.js `24.17.0` → `24.19.0` (LTS "Krypton"), Jib `3.5.3` → `3.5.4`, and Vitest `4.1.9` → `4.1.10`.
- Upgrade Angular from `22.0.x` to the `22.1` train: runtime packages to `22.1.0`, `@angular/build` and `@angular/cli` to `22.1.2`. The two groups are pinned independently because Angular publishes them on separate patch cadences; pinning both to the same patch produces an unsatisfiable peer set.
- Keep TypeScript at `6.0.3`. Angular 22.1 still pins `typescript: ">=6.0 <6.1"`, so the published TypeScript 7.0.x is out of range rather than a missed update.
...
- Clear the dev-only build-toolchain advisory backlog: 16 advisories (1 critical, 8 high, 4 moderate, 3 low) down to 3 moderate. Upgrading to the Angular 22.1 train cleared `piscina` (high, prototype-pollution gadget → RCE), `@babel/core`, `@angular/build`, and `@angular/compiler-cli`; a non-breaking `npm audit fix` cleared the critical `tar` chain (PAX path type confusion, decompression DoS, negative-entry-size infinite loop) plus high-severity `brace-expansion`, `fast-uri`, `immutable`, `ip-address`, `postcss`, and `undici`. The production gate stayed at `0` throughout — none of this ever reached the JAR or container image.
- The 3 residual moderate advisories (`@angular/cli`, `@modelcontextprotocol/sdk`, `@hono/node-server`) enter through `@angular/cli`'s bundled MCP server integration and have no forward fix. npm's only proposed remedy is a semver-major **downgrade** to `@angular/cli@21.0.4`, which was deliberately refused; they are gated on upstream Angular bumping its MCP SDK.
